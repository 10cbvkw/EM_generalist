...
def main():
    ...
    parser.add_argument('--output', type=str, required=True, help='Path to save output deblurred image')
    ...
    try:
        blurred_image = Image.open(args.input).convert('L')  # Convert to grayscale
        blurred_tensor = torch.from_numpy(np.array(blurred_image)).float() * 2 / 255.0 - 1
